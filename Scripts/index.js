...
function controlSelect(arg) {
    ...
    if (location.pathname.slice(- 1) !== '/' && location.hash !== '#/' + theme) {
        var count;
        if ((location.origin.indexOf('ej2npmci.azurewebsites') !== -1) && location.pathname.split('/').length >= 7) {
            count = 7;
        }
        else if ((location.origin.indexOf('ej2.syncfusion') !== -1) && location.pathname.split('/').length >= 6) {
            count = 6;
        }
