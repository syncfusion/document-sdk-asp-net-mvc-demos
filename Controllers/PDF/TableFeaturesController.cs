...
namespace EJ2MVCSampleBrowser.Controllers.PDF
{
    public partial class PdfController : Controller
    {
        ...
        public ActionResult TableFeatures(string InsideBrowser)
        {
            ...
            for (int i = 0; i < pdfGrid.Rows.Count; i++)
            {
                if (i % 2 == 0)
                {
                    ...
                    cell.Style.BackgroundImage = new PdfBitmap(ResolveApplicationImagePath(string.Format("{0}.jpg", cell.Value.ToString().ToLower())));
                    cell.Value = "";
        ...
        {
            DataSet dataSet = new DataSet();
            dataSet.ReadXml(ResolveApplicationDataPath("Report.xml"));
            return dataSet;
        }
        /// <summary>
        ...
        void pdfLightTable_BeginCellLayout(object sender, BeginCellLayoutEventArgs args)
        {
            if (args.RowIndex > -1 && args.CellIndex > -1)
            {
                ...
                {
                    args.Skip = true;
                    PdfBitmap img = new PdfBitmap(ResolveApplicationImagePath(string.Format("{0}.jpg", args.Value.ToString().ToLower())));
